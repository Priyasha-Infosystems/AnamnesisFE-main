...
export class MedicineEditComponent implements OnInit {
  ...
  async ngOnInit(){
    this.commonService.getRestrictedDrugsList()
    await this.store.pipe(select('utilityState')).subscribe(async val => {
  ...
  optionSelected= async()=>{
    const reqData: any = {
      apiRequest: { medicineCode: this.medicineCode}
    }
    await this.medicineDetailsService.getMedicineDetails(reqData)
      .then(async (res: any) => {
        if (!this.commonService.isApiError(res)) {
          ...
          medicineDetails.safetyMeasures= medicineDetails.safetyMeasures?this.safetyMeasuresParseXml(medicineDetails.safetyMeasures):null
  ...
  medicineFormReset(){
    ...
    this.medicineEntryForm.get('safetyMeasures')?.get('Alcohol')?.setValue('');
    this.medicineEntryForm.get('safetyMeasures')?.get('Pregnancy')?.setValue('');
    this.medicineEntryForm.get('safetyMeasures')?.get('Kidney')?.setValue('');
    this.medicineEntryForm.get('safetyMeasures')?.get('Allergy')?.setValue('');
    this.medicineEntryForm.get('safetyMeasures')?.get('Pediatrics')?.setValue('');
    this.medicineEntryForm.get('safetyMeasures')?.get('Driving')?.setValue('');
    this.medicineEntryForm.get('safetyMeasures')?.get('Breastfeeding')?.setValue('');
    this.medicineEntryForm.get('safetyMeasures')?.get('Liver')?.setValue('');
    this.medicineEntryForm.get('safetyMeasures')?.get('HeartDisease')?.setValue('');
    this.medicineEntryForm.get('safetyMeasures')?.get('Other')?.setValue('');
  ...
  save(data: any, isValid: boolean) {
    ...
    if (isValid) {
      ...
      this.toastr.error("Medicine form is not valid. Please fill the form with proper data");
    }
  }
