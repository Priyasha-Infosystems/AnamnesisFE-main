...
@Component({
  selector: 'app-labtest-report-creat',
  templateUrl: './labtest-report-creat.component.html',
  styleUrls: ['./labtest-report-creat.component.css']
})
export class LabtestReportCreatComponent implements OnInit {
  @Output()
  ...
  getDiagnosticCentreDetails(){
    const reqData: any = {
     ...
     this.labtestReportUploadService.GetDiagnosticCentreDetails(reqData)
      .then(async (res: any) => {
    ...
    if (data.length) {
      data.forEach((laboratoryTestReportDetails: any, laboratoryTestReportDetailsIndex: number) => {
        const templaboratoryTestReportDetails = {
          ...
          patientSearch: false,
          laboratoryTestDate: new Date(laboratoryTestReportDetails.laboratoryTestDate),
          testPageList: [],
          isAdd: false,
          isChange: false,
          isDelete: false,
          ...
          if (!tempLaboratoryTestReportPage.resultType) {
            ...
            laboratoryTestReportPage.laboratoryTestResultList.forEach((laboratoryTestResult: any, laboratoryTestResultIndex: number) => {
              const tempLaboratoryTestResult = {
                ...
                testResultChar: laboratoryTestResult.laboratoryTestValueChar,
                ...
                isNumeric: laboratoryTestResult.numeric,
              ...
              laboratoryTestResult.laboratoryTestComponentResultList.forEach((laboratoryTestComponent: any, laboratoryTestComponentIndex: number) => {
                const tempLaboratoryTestComponent: any = {
                  ...
                  isNumeric: laboratoryTestComponent.numeric,
      ...
      this.testReportList().at(0).get('diagnosticCentreID')?.setValue(this.healthClinicDetails.diagnosticCentreID)
      this.testReportList().at(0).get('diagnosticCentreName')?.setValue(this.healthClinicDetails.diagnosticCentreName)
      this.testReportList().at(0).get('diagnosticCentreName')?.disable();
      this.testReportList().at(0).get('laboratoryTestDate')?.setValue(new Date());
    }
    this.onFormChange(0)
  }
  ...
  addAnotherTestReport() {
    ...
    // this.labtestList(testReportIndex,testPageIndex).push(this.addLabTestGroup());
    // const labtestIndex = this.labtestList(testReportIndex,testPageIndex).length -1;
    // this.labtestList(testReportIndex,testPageIndex).at(labtestIndex).get('testComponentInd')?.setValue(true);
    // this.componentList(testReportIndex,testPageIndex,labtestIndex).push(this.addComponentGroup());
    // const componentIndex = this.componentList(testReportIndex,testPageIndex,labtestIndex).length -1;
    // this.componentList(testReportIndex,testPageIndex,labtestIndex).at(componentIndex).get('componentName')?.setValue('test component');
  }
          ...
          await this.labtestReportUploadService.searchDiagnosticCentre(reqData)
            .then(async (res: any) => {
              if (!this.commonService.isApiError(res) && res.apiResponse.diagnosticCentreCount > 0) {
                this.diagnosticCentreList = res.apiResponse.diagnosticCentreSearchDetailsList;
              } else {
  ...
  labtestNumericResultChange(testReportIndex: number, testPageIndex: number, labtestIndex: number, value: string, labtestIsAdd: any, testPageIssAdd: boolean, testReportIssAdd: boolean) {
    if (value.length) {
      if (!this.commonService.checkDecimalNumber(value)) {
        const oldVal = value
        this.labtestList(testReportIndex, testPageIndex).at(labtestIndex).get('testResult')?.setValue(oldVal.substring(0, oldVal.length - 1))
      }
    }
    if (!labtestIsAdd) {
